...
export function createSetMasterTool(config) {
    const logger = getLogger();
    return {
        name: "qlc_set_master",
        description: "Set the QLC+ master dimmer (grand master) value. Range is 0-1 (normalized), where 1 is full brightness.",
        ...
        cb: async (input) => {
            ...
            try {
                ...
                const result = await sendOsc({ path: "/vc/master", args: [value] }, { dryRun: config.qlcDryRun }, config);
...
export function createBlackoutTool(config) {
    const logger = getLogger();
    return {
        name: "qlc_blackout",
        description: "Trigger QLC+ blackout (fade all lights to 0). This is a safe, emergency-grade tool for immediate darkness.",
        schema: z.object({}),
        cb: async () => {
            logger.debug("Tool: qlc_blackout");
            try {
                // Blackout is typically /vc/blackout
                const result = await sendOsc({ path: "/vc/blackout", args: [1] }, { dryRun: config.qlcDryRun }, config);
...
export function createPanicTool(config) {
    const logger = getLogger();
    return {
        name: "qlc_panic",
        description: "Trigger QLC+ panic mode (emergency stop - instantly kills all lighting). Use only in emergencies.",
        schema: z.object({}),
        cb: async () => {
            logger.debug("Tool: qlc_panic");
            try {
                // Panic is typically /vc/panic
                const result = await sendOsc({ path: "/vc/panic", args: [1] }, { dryRun: config.qlcDryRun }, config);
...
export function createSetColorWashTool(config) {
    ...
    return {
        ...
        description: "Set a color wash using predefined colors (red, green, blue, amber, white, purple, cyan, magenta, yellow). Optionally specify DMX channels.",
        ...
        cb: async (input) => {
            ...
            if (!redChannel || !greenChannel || !blueChannel) {
                try {
                    const result = await sendOsc({ path: "/vc/master", args: [1] }, { dryRun: config.qlcDryRun }, config);
                    return text(`Color wash applied (mode: master). For RGB control, specify redChannel, greenChannel, blueChannel.`);
                }
                catch (err) {
                    const message = err instanceof Error ? err.message : String(err);
