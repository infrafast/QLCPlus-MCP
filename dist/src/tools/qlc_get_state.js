...
export const GetStateInputSchema = z.object({
    freshnessSeconds: optionalInt(z.number().int().min(1).max(300)).describe("How recent QLC+ feedback must be to count as live, in seconds."),
});
export function createGetStateTool() {
    const logger = getLogger();
    return {
        name: "qlc_get_state",
        description: "Report QLC+ native connection lifecycle and the temporary OSC rollback state. Native readiness requires authorization and a validated current project inventory.",
