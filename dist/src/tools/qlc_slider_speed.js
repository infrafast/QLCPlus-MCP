...
export function createSliderSetTool(config) {
    const logger = getLogger();
    return {
        name: "qlc_slider_set",
        description: "Set a QLC+ slider value (0-1 normalized range). Specify either the logical widget name or direct OSC path.",
        schema: SliderSetInputSchema,
        cb: async (input) => {
            logger.debug("Tool: qlc_slider_set", input);
            const { widgetName, oscPath, value } = input;
            if (value < 0 || value > 1) {
                return error(`Invalid slider value: ${value}. Expected 0-1 (normalized).`);
            }
            ...
            try {
                const result = await sendOsc({ path: resolution.path, args: [value] }, { dryRun: config.qlcDryRun }, config);
...
export function createSpeedSetTool(config) {
    const logger = getLogger();
    return {
        name: "qlc_speed_set",
        description: "Set the speed of a QLC+ speed dial or chase. Value should be in BPM (beats per minute).",
        ...
        cb: async (input) => {
            ...
            try {
                ...
                const result = await sendOsc({ path: resolution.path, args: [normalizedSpeed] }, { dryRun: config.qlcDryRun }, config);
...
//# sourceMappingURL=qlc_slider_speed.js.map