...
export function nativeSessionKey(customKey) {
    if (!customKey)
        return NATIVE_DEFAULT_KEY;
...
function simpleCryptCrc(data) {
    const nul = data.indexOf(0);
    return crc16(nul >= 0 ? data.subarray(0, nul) : data);
}
function keyParts(key) {
    return Array.from({ length: 8 }, (_, index) => Number((key >> BigInt(index * 8)) & 0xffn));
}
export function encryptNativePayload(clearPayload, key) {
    let flags = 0;
...
export function decryptNativePayload(ciphertext, key, maximumSize = MAX_NATIVE_DECRYPTED_PAYLOAD) {
    ...
    if ((flags & ~0x07) !== 0 || (flags & 0x02 && flags & 0x04)) {
        throw new Error(`Unsupported QLC+ SimpleCrypt flags 0x${flags.toString(16)}`);
    }
    ...
    if (flags & 0x02) {
        if (payload.length < 2)
            throw new Error("Truncated QLC+ SimpleCrypt CRC");
...
export function nativeInt(value) {
    const section = Buffer.allocUnsafe(5);
    section[0] = NativeSectionType.Integer;
    section.writeUInt32BE(value >>> 0, 1);
    return section;
}
export function nativeBoolean(value) {
    return Buffer.from([NativeSectionType.Boolean, value ? 1 : 0]);
}
export function nativeString(value) {
    return nativeVariableSection(NativeSectionType.String, Buffer.from(value, "utf8"));
}
export function nativeByteArray(value) {
    return nativeVariableSection(NativeSectionType.ByteArray, value);
}
function nativeVariableSection(type, value) {
...
export function makeNativePacket(opcode, key, sections) {
    if (sections.length > 0xff)
        throw new Error("Too many QLC+ packet sections");
...
export function parseNativeSections(payload, count, requiredPrefixOnly = false) {
    const result = [];
    let position = 0;
    for (let index = 0; index < count; index += 1) {
        ...
        if (type === NativeSectionType.Boolean) {
        ...
        else if (type === NativeSectionType.String ||
            ...
            position += length;
            result.push(type === NativeSectionType.String
...
//# sourceMappingURL=nativeCodec.js.map