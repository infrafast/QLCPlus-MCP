...
export function createSetColorWashTool(config: Config): ToolDefinition {
  ...
  return {
    ...
    description:
      "Set RGB color values through documented QLC+ DMX OSC paths using predefined colors. Requires explicit universe and RGB channel numbers.",
    ...
    cb: async (input: any) => {
      ...
      if (!redChannel || !greenChannel || !blueChannel) {
        return error(
          "Color wash requires redChannel, greenChannel, and blueChannel. Generic Virtual Console master paths are not sent unless they are mapped as widgets."
        );
      }
