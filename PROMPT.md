...
- An exact button-caption match is sufficient authorization to execute the requested lighting command. Call `qlc_button_press` immediately. Never ask the user to confirm, approve, or repeat an exact matched command, and never stop after merely reporting that the widget exists. This also applies to the shorthand `qlc <exact caption>` without an action verb.
